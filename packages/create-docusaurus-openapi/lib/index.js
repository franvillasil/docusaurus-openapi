...
function getTemplateInstallPackage(template, originalDirectory) {
  ...
  if (template) {
    ...
    if (match) {
      templateToInstall = `file:${path_1.default.resolve(
        originalDirectory,
        match[1]
      )}`;
    } else if (
      ...
      const scope =
        (packageMatch === null || packageMatch === void 0
          ? void 0
          : packageMatch[1]) || "";
      const templateName =
        (packageMatch === null || packageMatch === void 0
...
function getPackageInfo(installPackage) {
  ...
  if (match) {
    ...
    const { name, version } = require(path_1.default.join(
      installPackagePath,
      "package.json"
    ));
    return { name, version };
...
function createPackageJson(appPath, templateName) {
  ...
  const templatePath = path_1.default.dirname(
  ...
  fs_extra_1.default.writeFileSync(
    path_1.default.join(appPath, "package.json"),
    JSON.stringify(appPackage, null, 2) + os_1.default.EOL
  );
}
async function init(rootDir, siteName, reqTemplate, cliOptions = {}) {
  ...
  let template =
    reqTemplate !== null && reqTemplate !== void 0
      ? reqTemplate
      : RecommendedTemplate;
  ...
  if (isValidGitRepoUrl(template)) {
    ...
    if (
      shelljs_1.default.exec(`git clone --recursive ${template} ${dest}`, {
        silent: true,
      }).code !== 0
    ) {
    ...
    fs_extra_1.default.existsSync(
      path_1.default.resolve(process.cwd(), template)
    )
  ) {
    ...
    try {
      ...
      logger_1.default
        .error`Copying local template path=${templateDir} failed!`;
      throw err;
    }
  } else {
    ...
    fs_extra_1.default.writeFileSync(
      path_1.default.join(dest, "package.json"),
      JSON.stringify(packageJson, null, 2) + os_1.default.EOL
    );
  ...
  const cdpath =
    path_1.default.join(process.cwd(), name) === dest
      ? name
