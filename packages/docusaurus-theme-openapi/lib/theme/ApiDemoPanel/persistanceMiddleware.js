...
function createPersistanceMiddleware(options) {
  const persistanceMiddleware = (storeAPI) => (next) => (action) => {
    ...
    const storage = (0, _storageUtils.createStorage)(
      options === null || options === void 0 ? void 0 : options.authPersistance
    );
    ...
    if (action.type === _slice.setSelectedAuth.type) {
      if (state.auth.selected) {
        storage.setItem(
          (0, _storageUtils.hashArray)(Object.keys(state.auth.options)),
          state.auth.selected
        );
      }
    }
    ...
    if (action.type === _slice2.setServer.type) {
      ...
      if (
        (_state$server$value = state.server.value) !== null &&
        _state$server$value !== void 0 &&
        _state$server$value.url
      ) {
        var _state$server$value2;

        // FIXME What to use as key?
        storage.setItem(
          `docusaurus.openapi.server/${
            (_state$server$value2 = state.server.value) === null ||
            _state$server$value2 === void 0
              ? void 0
              : _state$server$value2.url
          }`,
    ...
    if (action.type === _slice2.setServerVariable.type) {
      ...
      if (
        (_state$server$value3 = state.server.value) !== null &&
        _state$server$value3 !== void 0 &&
        _state$server$value3.url
      ) {
        var _state$server$value4;

        storage.setItem(
          `docusaurus.openapi.server/${
            (_state$server$value4 = state.server.value) === null ||
            _state$server$value4 === void 0
              ? void 0
              : _state$server$value4.url
          }`,
          JSON.stringify(state.server.value)
        );
      }
