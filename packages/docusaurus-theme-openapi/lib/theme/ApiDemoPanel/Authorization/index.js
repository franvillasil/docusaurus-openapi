...
function LockButton({ mode, children, style, ...rest }) {
  return (
    <button
      className={(0, _clsx.default)("button", "button--primary", {
  ...
  if (editing) {
    return (
      <div className={_stylesModule.default.optionsPanel}>
        {optionKeys.length > 1 && (
          <_FormItem.default label="Security Scheme">
            <_FormSelect.default
              options={optionKeys}
              value={selected}
              onChange={(e) => {
        ...
        {selectedAuth.map((a) => {
            ...
            return (
              ...
              >
                <_FormTextInput.default
                  ...
                  value={
                    (_data$a$key$token = data[a.key].token) !== null &&
                    _data$a$key$token !== void 0
                      ? _data$a$key$token
                      : ""
                  }
                  onChange={(e) => {
          ...
          if (a.type === "http" && a.scheme === "basic") {
            ...
            return (
              <_react.default.Fragment key={selected + "-basic"}>
                <_FormItem.default label="Username">
                  <_FormTextInput.default
                    placeholder="Username"
                    value={
                      (_data$a$key$username = data[a.key].username) !== null &&
                      _data$a$key$username !== void 0
                        ? _data$a$key$username
                        : ""
                    }
                    onChange={(e) => {
                      const value = e.target.value.trim();
                ...
                <_FormItem.default label="Password">
                  <_FormTextInput.default
                    ...
                    password
                    value={
                      (_data$a$key$password = data[a.key].password) !== null &&
                      _data$a$key$password !== void 0
                        ? _data$a$key$password
                        : ""
                    }
                    onChange={(e) => {
                      const value = e.target.value.trim();
          ...
          if (a.type === "apiKey") {
            return (
              <_FormItem.default label={a.name} key={a.key + "-apiKey"}>
                <_FormTextInput.default
                  placeholder={a.name}
                  onChange={(e) => {
                    const value = e.target.value.trim();
